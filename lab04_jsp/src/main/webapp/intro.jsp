...
<%-- page 지시문 trimDirectiveWhitespaces 속성:
    ...
    - 빈 줄을 삭제할 것인지(true), 아닌 지(false: 기본값)를 설정하는 속성.
--%>
    
<%-- JSP 주석
1. Servlet(Server + Applet): WAS에서 실행되는, 요청을 처리하고 응답을 보내는 작은 자바 프로그램.
    (1) 서블릿 객체의 생성과 괸리, 서블릿 메서드 호출은 WAS가 담당.
    (2) 서블릿 동작 방식:
...
3. JSP의 구성 요소(문법)
    ...
    (2) 지시문(directive): <%@ ... %> JSP 페이지 설정, 컨텐트 타입, 인코딩, 옵션들을 설정. Java import 구문.
    (3) 선언문(declaration): <%! ... %> JSP 파일이 java 코드로 변환될 때, 클래스의 필드/메서드 선언 부분.
    (4) 스크립트릿(scriptlet): <% ... %> JSP 파일이 java 코드로 변환될 때, _jspService(req, resp) 메서드 안에 포함되는 자바 코드.
        - 지역 변수 선언 & 초기화, 메서드 호출, 조건문, 반복문, ...
    (5) 식, 표현식(expression): <%= ... %> JSP 파일이 java 코드로 변환될 때, out.write() 메서드의 아규먼트로 전달되는 값.
        - HTML 코드에 문자열을 삽입.
--%>    
