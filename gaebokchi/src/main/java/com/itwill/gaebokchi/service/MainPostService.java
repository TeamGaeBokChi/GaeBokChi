...
//	}
	...
	public List<MainPostListDto> getPagedPostsByUserid(int page, String author, int pageSize) {
		int startRow = (page - 1) * pageSize;
		int endRow = page * pageSize;
		return postDao.getPostListByUserid(startRow, author, endRow).stream().map(MainPostListDto::fromEntity)
				.collect(Collectors.toList());
	}
	...
	public int getTotalPostCountByUserid(String author) {
		return postDao.getTotalCountByUserid(author);
	}
